...
int showAgency(Agency *agency)
{
	...
	vector<MenuHelper::Option> options = {
		...
		{"3", "Faturacao"},
		{"4", "Utilitarios"},
		{"5", "Guardar alteracoes em disco"},
		{"x", "Sair (guarda alteracoes em disco)"},
	};
	...
	do
	{
		...
		MenuHelper::printLeftPaddedString("Nome  : " + agency->getName(), LEFTPAD);
		MenuHelper::printLeftPaddedString("NIF   : " + to_string(agency->getNif()), LEFTPAD);
		MenuHelper::printLeftPaddedString("URL   : " + agency->getWebsiteURL(), LEFTPAD);
		MenuHelper::printLeftPaddedString("Morada: " + agency->getAddress().toString(), LEFTPAD);
